...
check_syntax "$S"
check_contains "$S" "tos accept" "accepts Anaconda channel ToS"
check_contains "$S" "conda.*create|\\\$CONDA_EXE.*create" "creates conda env"
check_contains "$S" "conda.*run|\\\$CONDA_EXE.*run" "runs commands via conda run (non-interactive safe)"
check_contains "$S" "conda.*run.*pip install|\\\$CONDA_EXE.*run.*pip install" "installs python deps via conda run"
